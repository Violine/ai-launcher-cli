...
- [ ] FR-101: Запрос API-ключа при первом запуске
- [ ] FR-102: Маскировка ввода ключа (*)
- [ ] FR-103: Валидация формата ключа перед сохранением
- [ ] FR-104: Ошибка и повторный ввод при невалидном ключе
- [x] FR-105: Ключ в ~/.config/ai-launcher/config.yaml
- [x] FR-106: Права доступа 0600 на файл конфигурации
- [ ] FR-107: Сброс ключа через интерфейс (F7)

### 3.2. Получение списка моделей (API Client)

- [ ] FR-201: Запрос к API за доступными моделями после ввода ключа
- [ ] FR-202: Индикатор загрузки с анимацией
...
Отдельные пакеты `internal/executor`, `internal/telemetry`, `internal/api` (LLM Proxy) и при необходимости `internal/tools` (Tool Registry) появятся при реализации соответствующих FR.
